...
async function doMethodWithTodoById(id = null, method = "get") {
  try {
    if (method === "get" && id === null) {
      return await fetch(`${baseUrl}/todos`).then((res) => res.json());
    } else if (method === "get") {
      return await fetch(`${baseUrl}/todos/${id}`).then((res) => res.json());
    } else {
      return await fetch(`${baseUrl}/todos/${id}`, method).then((res) =>
        res.json()
      );
    }
  } catch (err) {
...
export { create, patch, put, deletetodos, doMethodWithTodoById };
